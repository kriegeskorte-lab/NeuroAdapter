...
class AffinityProjModel(torch.nn.Module):
    """Affinity Projection Model"""
    def __init__(self, h_featmap, w_featmap, target_featmap):
        super().__init__()
        self.proj = torch.nn.Linear(h_featmap*w_featmap, target_featmap)

    def forward(self, aff_map):
        proj_aff_map = self.proj(aff_map)
        return proj_aff_map
...
#     """Projection Model producing multiple IP tokens per image."""
#     def __init__(
#         self,
#         cross_attention_dim: int = 768,
#         clip_embeddings_dim: int = 768,
#         clip_extra_context_tokens: int = 4, #not used in this class, but kept for compatibility
#     ):
#         super().__init__()
#         self.cross_attention_dim       = cross_attention_dim
#         self.clip_extra_context_tokens = clip_extra_context_tokens

#         # Two-layer MLP with GELU, projecting → T × D
